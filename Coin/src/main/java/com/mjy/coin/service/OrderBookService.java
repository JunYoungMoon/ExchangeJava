...
public class OrderBookService {
    ...
    public void initializeBuyOrderBook(String symbol) {
        buyOrderBooks.putIfAbsent(symbol, new TreeMap<>(Comparator.reverseOrder()));
    }

    // 초기 매도 주문 호가 트리 생성 메서드
    public void initializeSellOrderBook(String symbol) {
        sellOrderBooks.putIfAbsent(symbol, new TreeMap<>());
    }

    // 초기 매수 주문 호가 추가
    public void addBuyOrderBook(String symbol, CoinOrderDTO order){
        BigDecimal price = processBigDecimal(order.getOrderPrice());
        BigDecimal amount = processBigDecimal(order.getQuantity());
        addOrderToBook(buyOrderBooks.get(symbol), price, amount);
    }

    // 초기 매도 주문 호가 추가
    public void addSellOrderBook(String symbol, CoinOrderDTO order){
        BigDecimal price = processBigDecimal(order.getOrderPrice());
        BigDecimal amount = processBigDecimal(order.getQuantity());
        addOrderToBook(sellOrderBooks.get(symbol), price, amount);
    }
    ...
    // 주문 체결 시 수량 업데이트
    public void updateOrderBook(String symbol, CoinOrderDTO order, boolean isBuy, boolean isAdd) {
        if (isBuy) {
            if (isAdd) {
                addOrderToBook(buyOrderBooks.get(symbol), order.getOrderPrice(), order.getQuantity());
            } else {
                subtractOrderFromBook(buyOrderBooks.get(symbol), order.getOrderPrice(), order.getQuantity());
            }
        } else {
            if (isAdd) {
                addOrderToBook(sellOrderBooks.get(symbol), order.getOrderPrice(), order.getQuantity());
            } else {
                subtractOrderFromBook(sellOrderBooks.get(symbol), order.getOrderPrice(), order.getQuantity());
            }
        }
    }

    // 호가 리스트 조회 (예: 상위 10개)
    public Map<BigDecimal, BigDecimal> getTopNBuyOrders(String symbol, int n) {
        return buyOrderBooks.getOrDefault(symbol, new TreeMap<>()).entrySet().stream()
                .limit(n)
                .collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
    }

    public Map<BigDecimal, BigDecimal> getTopNSellOrders(String symbol, int n) {
        return sellOrderBooks.getOrDefault(symbol, new TreeMap<>()).entrySet().stream()
                .limit(n)
                .collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
    }
    ...
    public Map<String, Map<BigDecimal, BigDecimal>> getTopNOrders(String symbol, int n) {
        Map<String, Map<BigDecimal, BigDecimal>> topOrders = new HashMap<>();

        // 매수 주문
        Map<BigDecimal, BigDecimal> topBuyOrders = getTopNBuyOrders(symbol, n);
        // 매도 주문
        Map<BigDecimal, BigDecimal> topSellOrders = getTopNSellOrders(symbol, n);
    ...
    public void printOrderBook(String symbol) {
        Map<BigDecimal, BigDecimal> topBuyOrders = getTopNBuyOrders(symbol, 10);
        Map<BigDecimal, BigDecimal> topSellOrders = getTopNSellOrders(symbol, 10);
